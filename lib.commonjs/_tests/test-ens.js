...
describe("Test ENSv2 migration", function () {
    ...
    it("getUniversal", async () => {
        const addr = await index_js_1.EnsResolver.getUniversalResolverAddress(provider);
        assert_1.default.equal(addr, "0xeEeEEEeE14D718C2B47D9923Deab1335E144EeEe");
    });
    // Tests from Raffy
    const testsResolver = [
        ["raffy.eth", "0x84c5AdB77dd9f362A1a3480009992d8d47325dc3"],
        ["apple.wiki.tog.raffy.eth", "0x7CE6Cf740075B5AF6b1681d67136B84431B43AbD"],
        ["cap.eth", "0xF29100983E058B709F3D539b0c765937B804AC15"],
        ["vitalik.eth", "0x231b0Ee14048e9dCcD1d247744d114a4EB5E8E63"],
    ...
    for (const [name, addr] of testsResolver) {
        it(`fromName (${name})`, async () => {
            ...
            assert_1.default.ok(resolver);
            assert_1.default.equal(resolver.address, addr);
            assert_1.default.equal(getCallCount(), 1);
        });
    }
    const testsAddr = [
        {
    ...
    for (const { title, name, address, coinType, test } of testsAddr) {
        it(`ENS Test: ${title}`, async () => {
            if (test.forward) {
                ...
                assert_1.default.equal(addr, address);
            }
            if (test.reverse) {
                assert_1.default.ok(typeof (address) === "string");
                const lookupName = await provider.lookupAddress(address, coinType);
                assert_1.default.equal(lookupName, name);
            }
            if (test.text) {
                const resolver = await provider.getResolver(name);
                assert_1.default.ok(resolver);
                const value = await resolver.getText(test.text.key);
                assert_1.default.equal(value, test.text.expected);
            }
            if (test.content) {
                const resolver = await provider.getResolver(name);
                assert_1.default.ok(resolver);
                const value = await resolver.getContentHash();
                assert_1.default.equal(value, test.content);
            }
        });
    }
});
//# sourceMappingURL=test-ens.js.map