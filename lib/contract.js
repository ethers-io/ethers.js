    ...
    return {
        encode: function(value) {
            ...
            if (signed) {
                ...
                //value = value.toTwos(size * 8).and(bitmasks[size]).fromTwos(size * 8).toTwos(256);
//            } else { //if (value.isNeg()) {
//                value = value.toTwos(size * 8).and(bitmasks[size]);
//            } else {
//                value = value.and(bitmasks[size]);
        ...
        decode: function(data, offset) {
            ...
            if (signed) {
                ...
                value = value.fromTwos(size * 8); //.toTwos(256); //mod((new utils.BN(2)).pow(new utils.BN(size * 8)))
            }
            return {
                consumed: 32,
                value: value,
