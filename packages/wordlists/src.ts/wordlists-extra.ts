...
export function attachEthers(ethers: any): void {
    if (_ethers) { throw new Error("already attached"); }
    _ethers = ethers;
}
export function getEthers(): any {
    if (_ethers) { throw new Error("no ethers attached; please use attachEthers"); }
    return _ethers;
}
