...
import type { JsonRpcIncomingMessage, JsonRpcPayload } from "./provider-jsonrpc.js";
...
export class SocketProvider extends JsonRpcApiProvider {
    ...
    async _processMessage(message: string): Promise<void> {
        const result: JsonRpcIncomingMessage = JSON.parse(message);
        if ("id" in result) {
            const promise = this.#callbacks.get(result.id);
            if (!promise) {
